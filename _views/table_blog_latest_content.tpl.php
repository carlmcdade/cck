	...
	foreach ($rows as $tr => $row)
	{
		...
		$output .= '<div style="" class="">';
			...
			if(isset($row) && !empty($row['content_title']))
			{
				...
				$output .= '<div style="padding:1em;background-color:#dee5ff;"><h1 style="" id="td-' . (isset($id) ? $id : '')  . $tr . '-' .  $tr . '" class="">' . (isset($row['content_title']) ? $row['content_title']:'') . ' </h1><br>by <span class="fs-5">' .$row['name'] ."</span></div>\n";
