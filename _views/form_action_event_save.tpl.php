...
if (!empty($_POST)) {
    ...
    //var_dump($value);
        // $post_name = 'Posted-'. $type;
            $output .= ' <label>'.$label.'</label><input value="'. $value.'" type="text" class="form-control" id="" name="content-type-field-'. $label.'" />' . "\n";
            $output .= '' . "\n";
        }
    ...
    if (!empty($eventFields)) {
        ...
        $count = 1;
            ...
            $output .= ' <input value="'.$eventFields['content_id'].'" type="hidden" class="form-control" id="" name="event_create[event][fields][content_id]" />' . "\n";
