...
//$output .= print_r($form,1);
	...
	foreach ($line as $tr => $row)
	{
			...
			if(!is_numeric($tr))
            {
                ...
                {   
                $output .= '<tr><td id="td-'. 
                    '" class="table-cells"> <label style="padding:.5rem;text-align:center;background-color:#e1e1e1;display:block;border-bottom: 1px #777 solid;">['.(isset($tr) ? $tr : '').']['. (isset($row)? print_r($item,1): 'no label') .
                    ...
                    '" type="text" class=" form-control" id="" name="'.(isset($field) ? $field: '').'"></td></tr>' . "\n";
                }
            }else{
		
				     $output .= '<tr><td id="td-'. 
				     '" class="table-cells"> <label style="padding:.5rem; text-align:center;background-color:#e1e1e1;display:block;border-bottom 1px #777 solid;">['.(isset($tr) ? $tr : '').']['. (isset($count)? $count: 'no label') .
...
<button class="btn btn-secondary" type="reset" formaction="?content/content_add_field/'.$tr.'">Reset</button>'."\n".'
</div></form>' . "\n";
print $output;
?>
